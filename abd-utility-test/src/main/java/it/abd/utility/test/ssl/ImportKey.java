...
Key format: openssl pkcs8 -topk8 -nocrypt -in YOUR.KEY -out YOUR.KEY.der -outform der

Format of the certificate: openssl x509 -in YOUR.CERT -out YOUR.CERT.der -outform der

Import key and certificate: java it.abd.utility.test.ssl.ImportKey YOUR.KEY.der YOUR.CERT.der

Caution: the old keystore.ImportKey-file is deleted and replaced with a keystore only containing YOUR.KEY
and YOUR.CERT. 
...
The name of the keystore file can be controlled by setting
    ...
    public static void main ( String args[]) {
        ...
        if (args.length < 2 || args.length>3) {
            System.out.println("Usage: ImportKey keyfile certfile [alias]");
            System.exit(0);
        } else {
            keyfile = args[0];
            certfile = args[1];
    ...
    ///usr/java/default/bin/keytool -importkeystore -srckeystore  /usr/java/jdk1.7.0_71/jre/lib/security/cacerts -destkeystore /opt/apache-servicemix-6.0.0/keystore/livornoEsbKeystore.jks -srcstorepass changeit -deststorepass skpass -srcalias addtrustexternalca
    public void importCertificateByAlias(String alias){
