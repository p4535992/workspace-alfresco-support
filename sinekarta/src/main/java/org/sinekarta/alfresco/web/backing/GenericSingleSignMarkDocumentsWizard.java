 ...
 *
 */
public class GenericSingleSignMarkDocumentsWizard extends BaseWizardBean {
	
	private static final long serialVersionUID = 1L;

	private static Logger tracer = Logger.getLogger(GenericSingleSignMarkDocumentsWizard.class);

	protected static final String BUNDLE_SINGLE_SIGN_TITLE = "singleSignMark_title";

	protected static final String BUNDLE_SUMMARY = "summary";
	protected static final String BUNDLE_GENERIC_SIGN_MARK = "bundle.sinekarta-genericSignMark";
	...
	protected transient ResourceBundle resourceBundle = ResourceBundle.getBundle(BUNDLE_GENERIC_SIGN_MARK, org.jenia.faces.util.Util.getFacesContext().getViewRoot().getLocale(), Util.class.getClassLoader());
	...
	private void init() {
		...
		do {
			...
			if (i==0) {
				documentNamePdf = documentNamePdf.substring(0, dotIdx) + Configuration.getInstance().getMarkedSuffix() + documentNamePdf.substring(dotIdx);
			} else {
				documentNamePdf = documentNamePdf.substring(0, dotIdx) + " ("+i+")" + Configuration.getInstance().getMarkedSuffix() + documentNamePdf.substring(dotIdx);
			}
	...
	protected String finishImpl(FacesContext context, String outcome)
		...
		try {
			...
			Action digitalSignatureTimestampMarkApply = su.getActionService().createAction(DocumentDigitalSignatureTimestampMarkApply.ACTION_NAME_DOCUMENT_DIGITAL_SIGNATURE_TIMESTAMP_MARK_APPLY);
			digitalSignatureTimestampMarkApply.setParameterValue(DocumentDigitalSignatureTimestampMarkApply.PARAM_CLIENT_AREA, su.getDataFromAppletSign());
			try {
				su.getActionService().executeAction(digitalSignatureTimestampMarkApply, selectedDocumentNodeParent, false, false);
			} catch(Throwable t) {
				tracer.error("Unable to apply sign to document : " + t.getMessage(),t);
				throw new MarkFailedException("Unable to apply sign to document : " + t.getMessage(),t);
			}
			...
			throw new MarkFailedException("Unable to apply sign to document : " + e.getMessage(), e);
		}
	...
	protected void prepareDataToAppletSign() {
		...
		try {
			...
			try {
				...
				throw new MarkFailedException("Unable to execute PDF/A conversion : " + t.getMessage(),t);
			}
			...
			try {
				...
				throw new MarkFailedException("Unable to prepare data for document sign : " + t.getMessage(),t);
			}
			...
			throw new MarkFailedException("Unable to calculate document fingerprint.", e);
		}
	...
	protected void prepareDataToAppletCertificateChoice() {
		...
		try {
			...
			throw new MarkFailedException("Unable to initialize digital signature alfresco action service : " + t.getMessage(),t);
		}
	...
	public String next() {
		...
		if (stepName.equals(STEP_NAME_CERTIFICATE_CHOICE)) {
			...
			if (su.getFileFolderService().searchSimple(folder, documentName)!=null) { 
				Util.addFatalMessage(BUNDLE_GENERIC_SIGN_MARK, "fileAlreadyExists", "fileAlreadyExistsDesc");
