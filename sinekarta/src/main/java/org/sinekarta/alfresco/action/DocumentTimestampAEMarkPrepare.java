 ...
 *
 */
public class DocumentTimestampAEMarkPrepare extends ActionExecuterAbstractBase implements InitializingBean {

	private static Logger tracer = Logger.getLogger(DocumentTimestampAEMarkPrepare.class);
	public static final String ACTION_NAME_DOCUMENT_TIMESTAMP_AEMARK_PREPARE = "sinekartaDocumentTimestampAEMarkPrepare";
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
		if (tracer.isDebugEnabled()) tracer.debug("DocumentTimestampAEMarkPrepare action, execution init");
		
		String clientArea = (String)action.getParameterValue(PARAM_CLIENT_AREA);
		if (clientArea==null) {
			tracer.error("no clientArea specified for aemark prepare.");
			throw new MarkFailedException("no clientArea specified for aemark prepare.");
		}
				
		String markArea = (String)action.getParameterValue(PARAM_MARK_AREA);
		if (markArea==null) {
			tracer.error("no markArea specified for aemark prepare.");
			throw new MarkFailedException("no markArea specified for aemark prepare.");
		}
		...
		DocumentTimestampAEMarkPrepareWorker execAsSinekartaAdmin = new DocumentTimestampAEMarkPrepareWorker(actionService, clientArea, markDocumentArea);
		
		Boolean result = AuthenticationUtil.runAs(execAsSinekartaAdmin, sinekartaAdminUserId);
		if (!result) {
			tracer.error("error on timestamp aemark prepare : " + execAsSinekartaAdmin.getKoReason());
			throw new MarkFailedException("error on timestamp aemark prepare : " + execAsSinekartaAdmin.getKoReason());
		}

		action.setParameterValue(PARAM_RESULT, execAsSinekartaAdmin.getRet());

		if (tracer.isDebugEnabled()) tracer.debug("DocumentTimestampAEMarkPrepare action, execution end");
	}
...
class DocumentTimestampAEMarkPrepareWorker implements RunAsWork<Boolean> {
	...
	public DocumentTimestampAEMarkPrepareWorker(ActionService actionService, String clientArea, MarkDocumentArea markDocumentArea) {
		super();
		this.actionService = actionService;
	...
	public Boolean doWork() throws Exception {
		if (tracer.isDebugEnabled()) tracer.debug("DocumentTimestampAEMarkPrepare action.doWork, execution init");
...
//			NodeRef tmpMarkFolder = new NodeRef(tmpStoreRef, markDocumentArea.getTmpMarkFolderPathNodeRefId());
			...
			tracer.error("DocumentTimestampAEMarkPrepare action.doWork, generic error", e);
			koReason="DocumentTimestampAEMarkPrepare action.doWork, generic error : " + e.getMessage();
			return false;
		}

		if (tracer.isDebugEnabled()) tracer.debug("DocumentTimestampAEMarkPrepare action.doWork, finished OK");
