...
/**
 * document digital signature and timestamp mark apply action.
 * this action will apply digital signature and timestamp mark to all document prepared for
 * timestamp mark service is configured on sinekarta-repository.properties
 * 
 ...
 *
 */
public class DocumentDigitalSignatureTimestampMarkApply extends ActionExecuterAbstractBase implements InitializingBean {

	private static Logger tracer = Logger.getLogger(DocumentDigitalSignatureTimestampMarkApply.class);
	public static final String ACTION_NAME_DOCUMENT_DIGITAL_SIGNATURE_TIMESTAMP_MARK_APPLY = "sinekartaDocumentDigitalSignatureTimestampMarkApply";
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
		if (tracer.isDebugEnabled()) tracer.debug("DocumentDigitalSignatureTimestampMarkApply action, execution init");
		
		String clientArea = (String)action.getParameterValue(PARAM_CLIENT_AREA);
		if (clientArea==null) {
			tracer.error("no clientArea specified for DocumentDigitalSignatureTimestampMarkApply.");
			throw new DigitalSignatureException("no clientArea specified for DocumentDigitalSignatureTimestampMarkApply.");
		}
		...
		for (DigitalSignatureDocument dsd : docs) {
			...
			// applying digital signature and timestamp mark using itext
			...
			try {
				PDFTools.signAndMark(dsd, sca.hexCertificateToX509Certificate(), is, os,
						Configuration.getInstance().getTsaUrl(), Configuration.getInstance().getTsaUser(), Configuration.getInstance().getTsaPassword());
		...
		if (tracer.isDebugEnabled()) tracer.debug("DocumentDigitalSignatureTimestampMarkApply action, execution end");
	}
