 ...
 *
 */
public class AEMarkDocumentPrepare extends ActionExecuterAbstractBase implements InitializingBean {

	private static Logger tracer = Logger.getLogger(AEMarkDocumentPrepare.class);
	public static final String ACTION_NAME_AEMARK_DOCUMENT_PREPARE = "sinekartaAEMarkDocumentPrepare";
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
		if (tracer.isDebugEnabled()) tracer.debug("DocumentAEMarkDocumentPrepare action, execution init");
		
		String markArea = (String)action.getParameterValue(PARAM_MARK_AREA);
		if (markArea==null) {
			tracer.error("no aemarkFileName specified for RCS mark prepare.");
			throw new MarkFailedException("no aemarkFileName specified for RCS mark prepare.");
		}
		...
		if (markDescription==null) {
			tracer.error("no markDescription specified for aemark prepare.");
			throw new MarkFailedException("no markDescription specified for aemark prepare.");
		}
				
		String markFileName = (String)action.getParameterValue(PARAM_MARK_FILENAME);
		if (markFileName==null) {
			tracer.error("no markFileName specified for aemark prepare.");
			throw new MarkFailedException("no markFileName specified for aemark prepare.");
		}
		...
		if (nodeService.getChildByName(markFolderNodeRef,ContentModel.ASSOC_CONTAINS, markFileName + Constants.EXTENSION_PDF)!=null) {
			tracer.error("A AEMark file with the specified name already exists in mark folder directory");
			throw new MarkDocumentAlreadyExistsException("A AEMark file with the specified name already exists in mark folder directory");
		}

		AEMarkDocumentPrepareWorker execAsSinekartaAdmin = new AEMarkDocumentPrepareWorker(contentService, 
		...
		if (!result) {
			tracer.error("error on aemark document prepare : " + execAsSinekartaAdmin.getKoReason());
			throw new MarkFailedException("error on aemark document prepare : " + execAsSinekartaAdmin.getKoReason());
		}
...
class AEMarkDocumentPrepareWorker implements RunAsWork<Boolean> {
	...
	public AEMarkDocumentPrepareWorker(ContentService contentService, 
	...
	public Boolean doWork() throws Exception {
		if (tracer.isDebugEnabled()) tracer.debug("DocumentRCSAEMarkPrepare action.doWork, execution init");
		...
		try {
			...
			if (tracer.isDebugEnabled()) tracer.debug("DocumentRCSAEMarkPrepare action.doWork, creazione file PDF/A di marca temporale in sinekarta temporary folder");
			...
			document.add(new Paragraph(Constants.IMPRONTE_AE, titolo));
			document.add(new Paragraph(Constants.DATA_APPOSIZIONE_MARCA_TEMPORALE + " : " + data, sottoTitolo));
			document.add(new Paragraph("\n",capitolo));
			
			// impronte tag.
			hd.startElement("","",Constants.IMPRONTE_AE,atts);
			...
			for (MarkDocument n : markDocumentArea.getDocuments()) {
				...
				tipoDocumento = (String)nodeService.getProperty(nodeRef, SinekartaModel.PROP_QNAME_MARK_DOCUMENT_TYPE);
				if (tipoDocumento==null || tipoDocumento.equals("")) {
					tracer.error("document type to which mark refers to unknown: " + tipoDocumento);
					throw new MarkFailedException("document type to which mark refers to unknown: " + tipoDocumento);
				} else {
					DocumentType dt = sinekartaDao.getDocumentType(Integer.parseInt(tipoDocumento));
					if (dt.getExternalCode()==null || dt.getExternalCode().trim().equals("")) {
						tracer.error("No external code found on documentType for type : " + dt.getDescription() + ", can not create xml for AE.");
						throw new MarkFailedException("No external code found on documentType for type : " + dt.getDescription() + ", can not create xml for AE.");
					}
					tipoDocumento = tipoDocumento + " - " + dt.getDescription();
				}
				
				atts.addAttribute("","",Constants.TIPI_DOCUMENTO_MARCATI,Constants.CDATA, tipoDocumento);
				idRiferimento = (String)nodeService.getProperty(nodeRef, SinekartaModel.PROP_QNAME_MARK_DOCUMENT_REFERENCE_ID);
				atts.addAttribute("","",Constants.ID_RIFERIMENTO,Constants.CDATA, idRiferimento);
				riferimentoTemporaleFirmaRCS = sdf.format((Date)nodeService.getProperty(nodeRef, SinekartaModel.PROP_QNAME_MARK_TIMESTAMP_RCS_SIGNATURE));
				atts.addAttribute("","",Constants.DATA_APPOSIZIONE_MARCA_TEMPORALE,Constants.CDATA,riferimentoTemporaleFirmaRCS);
				...
				document.add(new Paragraph(Constants.TIPI_DOCUMENTO_MARCATI + " : " + tipoDocumento, testo));
				document.add(new Paragraph(Constants.ID_RIFERIMENTO + " : " + idRiferimento, testo));
				document.add(new Paragraph(Constants.DATA_APPOSIZIONE_MARCA_TEMPORALE + " : " + riferimentoTemporaleFirmaRCS, testo));
				...
				impronta = (String)nodeService.getProperty(nodeRef, SinekartaModel.PROP_QNAME_MARK_FINGER_PRINT);
				...
				atts.clear();
				hd.startElement("","",Constants.TSR,atts);

				String tsr = null;
				tsr = (String)nodeService.getProperty(nodeRef, SinekartaModel.PROP_QNAME_MARK_TIMESTAMP_TOKEN);
				writeToTransformerHandler(hd,tsr);
				
				document.add(new Paragraph(Constants.TSR + " : " + tsr , testoEvidenziato));
				document.add(new Paragraph("\n",capitolo));
				
				hd.endElement("","",Constants.HASH);
				hd.endElement("","",Constants.IMPRONTA);
			}
			hd.endElement("","",Constants.IMPRONTE_AE);
			hd.endDocument();
			baos.flush();
			baos.close();
			...
			tracer.error("DocumentRCSAEMarkPrepare action.doWork, generic error", e);
			koReason="DocumentRCSAEMarkPrepare action.doWork, generic error : " + e.getMessage();
			return false;
		}
		
		if (tracer.isDebugEnabled()) tracer.debug("DocumentRCSAEMarkPrepare action.doWork, finished OK");
