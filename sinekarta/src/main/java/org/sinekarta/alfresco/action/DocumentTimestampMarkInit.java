 ...
 *
 */
public class DocumentTimestampMarkInit extends ActionExecuterAbstractBase implements InitializingBean {

	private static Logger tracer = Logger.getLogger(DocumentTimestampMarkInit.class);
	public static final String ACTION_NAME_DOCUMENT_TIMESTAMP_MARK_INIT = "sinekartaDocumentTimestampMarkInit";
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
		if (tracer.isDebugEnabled()) tracer.debug("DocumentTimestampMarkInit action, execution init");
		
		String markArea = (String)action.getParameterValue(PARAM_MARK_AREA);
		if (markArea==null) {
			tracer.error("no clientArea specified for mark sign.");
			throw new MarkFailedException("no clientArea specified for timestamp mark init.");
		}
		...
		DocumentTimestampMarkInitWorker execAsSinekartaAdmin = new DocumentTimestampMarkInitWorker(actionService, markArea, personService, 
		...
		if (!result) {
			tracer.error("error on timestamp mark init : " + execAsSinekartaAdmin.getKoReason());
			throw new MarkFailedException("error on timestamp mark init : " + execAsSinekartaAdmin.getKoReason());
		}

		action.setParameterValue(PARAM_RESULT, execAsSinekartaAdmin.getRet());

		if (tracer.isDebugEnabled()) tracer.debug("DocumentTimestampMarkInit action, execution end");
	}
...
class DocumentTimestampMarkInitWorker implements RunAsWork<Boolean> {
	...
	public DocumentTimestampMarkInitWorker(ActionService actionService, String markArea, 
	...
	public Boolean doWork() throws Exception {
		if (tracer.isDebugEnabled()) tracer.debug("DocumentRCSMarkSignPrepare action.doWork, execution init");
		...
		try {
			...
			tracer.error("DocumentRCSMarkSignPrepare action.doWork, generic error", e);
			koReason="DocumentRCSMarkSignPrepare action.doWork, generic error : " + e.getMessage();
			return false;
		}

		if (tracer.isDebugEnabled()) tracer.debug("DocumentRCSMarkSignPrepare action.doWork, finished OK");
