...
/**
 * document PU digital signature prepare action.
 ...
 *
 */
public class DocumentPUSignPrepare extends ActionExecuterAbstractBase implements InitializingBean {

	private static Logger tracer = Logger.getLogger(DocumentPUSignPrepare.class);
	public static final String ACTION_NAME_DOCUMENT_PU_SIGN_PREPARE = "sinekartaDocumentPUSignPrepare";
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
		if (tracer.isDebugEnabled()) tracer.debug("DocumentPUSignPrepare action, execution init");
		...
		DocumentPUSignPrepareWorker execAsSinekartaAdmin = new DocumentPUSignPrepareWorker(actionService, clientArea, actionedUponNodeRef);
		...
		if (!result) {
			tracer.error("error on pu prepare : " + execAsSinekartaAdmin.getKoReason());
			throw new SignFailedException("error on pu prepare : " + execAsSinekartaAdmin.getKoReason());
		}

		action.setParameterValue(PARAM_RESULT, execAsSinekartaAdmin.getRet());

	if (tracer.isDebugEnabled()) tracer.debug("DocumentPUSignPrepare action, execution end");
	}
...
class DocumentPUSignPrepareWorker implements RunAsWork<Boolean> {
	...
	public DocumentPUSignPrepareWorker(ActionService actionService, String clientArea, NodeRef actionedUponNodeRef) {
		super();
		this.actionService = actionService;
	...
	public Boolean doWork() throws Exception {
		if (tracer.isDebugEnabled()) tracer.debug("DocumentPUSignPrepare action.doWork, execution init");
		...
		try {
			...
			digitalSignaturePrepareAndAddDocument.setParameterValue(DocumentDigitalSignaturePrepareAndAddDocument.PARAM_SIGN_DESCRIPTION, Constants.PU_SIGN_REASON_PROCEDURA_DI_FIRMA_DIGITALE_PROPEDEUTICA_ALLA_CONSERVAZIONE_SOSTITUTIVA);
			...
			tracer.error("DocumentPUSignPrepare action.doWork, generic error", e);
			koReason="DocumentPUSignPrepare action.doWork, generic error : " + e.getMessage();
			return false;
		}
		
		if (tracer.isDebugEnabled()) tracer.debug("DocumentPUSignPrepare action.doWork, finished OK");
