...
import org.sinekarta.alfresco.webscript.request.AETimestampMarkInitRequest;
import org.sinekarta.alfresco.webscript.response.AETimestampMarkInitResponse;
...
public class AETimestampMarkInitWS extends DeclarativeWebScript {

	private static final String REQUEST_TYPE = "requestType";

	private static Logger tracer = Logger.getLogger(AETimestampMarkInitWS.class);
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		if (tracer.isDebugEnabled()) tracer.debug("webscript AETimestampMarkInitWS starting");
		
		Map<String, Object> model = new HashMap<String, Object>();
		
		try {
			String requestType = req.getParameter(REQUEST_TYPE);
			AETimestampMarkInitRequest r = null;
			if (requestType.equalsIgnoreCase("xml")) {
				r = AETimestampMarkInitRequest.fromXML(req.getContent().getInputStream());
			} else if (requestType.equalsIgnoreCase("json")) {
				r = AETimestampMarkInitRequest.fromJSON(req.getContent().getInputStream());
			} else {
			...
			Action documentTimestampMarkInit = actionService.createAction(DocumentTimestampAEMarkInit.ACTION_NAME_DOCUMENT_TIMESTAMP_AEMARK_INIT);
			documentTimestampMarkInit.setParameterValue(DocumentTimestampAEMarkInit.PARAM_MARK_AREA, r.getMarkDocumentArea());
			try {
			...
			AETimestampMarkInitResponse resp = new AETimestampMarkInitResponse();
			...
			tracer.error("Generic error on AETimestampMarkInitWS web script : " + e.getClass().getName() + " : " + e.getMessage(),e);
			throw new WebScriptException(Status.STATUS_NOT_ACCEPTABLE, "Generic error on AETimestampMarkInitWS web script : " + e.getClass().getName() + " : " + e.getMessage(),e);
		}
		
		if (tracer.isDebugEnabled()) tracer.debug("webscript AETimestampMarkInitWS finished");
