...
import org.sinekarta.alfresco.webscript.request.MarkDocumentPrepareRequest;
import org.sinekarta.alfresco.webscript.response.MarkDocumentPrepareResponse;
...
public class MarkDocumentPrepareWS extends DeclarativeWebScript {

	private static final String REQUEST_TYPE = "requestType";

	private static Logger tracer = Logger.getLogger(MarkDocumentPrepareWS.class);
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		...
		try {
			...
			MarkDocumentPrepareRequest r = null;
			if (requestType.equalsIgnoreCase("xml")) {
				r = MarkDocumentPrepareRequest.fromXML(req.getContent().getInputStream());
			} else if (requestType.equalsIgnoreCase("json")) {
				r = MarkDocumentPrepareRequest.fromJSON(req.getContent().getInputStream());
			} else {
			...
			Action markDocumentPrepare = actionService.createAction(MarkDocumentPrepare.ACTION_NAME_MARK_DOCUMENT_PREPARE);
			markDocumentPrepare.setParameterValue(MarkDocumentPrepare.PARAM_MARK_AREA, r.getMarkDocumentArea());
			markDocumentPrepare.setParameterValue(MarkDocumentPrepare.PARAM_MARK_DESCRIPTIONR, r.getMarkDescription());
			markDocumentPrepare.setParameterValue(MarkDocumentPrepare.PARAM_MARK_FILENAME, r.getMarkFileName());
			...
			MarkDocumentPrepareResponse resp = new MarkDocumentPrepareResponse();
