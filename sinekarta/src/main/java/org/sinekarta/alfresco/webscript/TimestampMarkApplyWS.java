...
import org.sinekarta.alfresco.webscript.request.TimestampMarkApplyRequest;
import org.sinekarta.alfresco.webscript.response.TimestampMarkApplyResponse;
...
public class TimestampMarkApplyWS extends DeclarativeWebScript {

	private static final String REQUEST_TYPE = "requestType";

	private static Logger tracer = Logger.getLogger(TimestampMarkApplyWS.class);
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		if (tracer.isDebugEnabled()) tracer.debug("webscript TimestampMarkApplyWS starting");
		
		Map<String, Object> model = new HashMap<String, Object>();
		
		try {
			String requestType = req.getParameter(REQUEST_TYPE);
			TimestampMarkApplyRequest r = null;
			if (requestType.equalsIgnoreCase("xml")) {
				r = TimestampMarkApplyRequest.fromXML(req.getContent().getInputStream());
			} else if (requestType.equalsIgnoreCase("json")) {
				r = TimestampMarkApplyRequest.fromJSON(req.getContent().getInputStream());
			} else {
			...
			if (r.getDigitalSignatureArea()==null || r.getDigitalSignatureArea().equals("")) { 
				tracer.error("No DigitalSignatureArea specified for TimestampMarkApplyWS action");
				throw new WebScriptException(Status.STATUS_BAD_REQUEST, "No DigitalSignatureArea specified for TimestampMarkApplyWS action");
			}
			...
			Action documentTimestampMarkApply = actionService.createAction(DocumentTimestampMarkApply.ACTION_NAME_DOCUMENT_RCS_MARK_SIGN_APPLY);
			documentTimestampMarkApply.setParameterValue(DocumentTimestampMarkApply.PARAM_MARK_AREA, r.getMarkDocumentArea());
			documentTimestampMarkApply.setParameterValue(DocumentTimestampMarkApply.PARAM_CLIENT_AREA, r.getDigitalSignatureArea());
			try {
			...
			TimestampMarkApplyResponse resp = new TimestampMarkApplyResponse();
			...
			tracer.error("Generic error on TimestampMarkApplyWS web script : " + e.getClass().getName() + " : " + e.getMessage(),e);
			throw new WebScriptException(Status.STATUS_NOT_ACCEPTABLE, "Generic error on TimestampMarkApplyWS web script : " + e.getClass().getName() + " : " + e.getMessage(),e);
		}
		
		if (tracer.isDebugEnabled()) tracer.debug("webscript TimestampMarkApplyWS finished");
