...
import org.sinekarta.alfresco.webscript.request.TimestampMarkPrepareRequest;
import org.sinekarta.alfresco.webscript.response.TimestampMarkPrepareResponse;
...
public class TimestampMarkPrepareWS extends DeclarativeWebScript {

	private static final String REQUEST_TYPE = "requestType";

	private static Logger tracer = Logger.getLogger(TimestampMarkPrepareWS.class);
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		if (tracer.isDebugEnabled()) tracer.debug("webscript TimestampMarkPrepareWS starting");
		
		Map<String, Object> model = new HashMap<String, Object>();
		
		try {
			String requestType = req.getParameter(REQUEST_TYPE);
			TimestampMarkPrepareRequest r = null;
			if (requestType.equalsIgnoreCase("xml")) {
				r = TimestampMarkPrepareRequest.fromXML(req.getContent().getInputStream());
			} else if (requestType.equalsIgnoreCase("json")) {
				r = TimestampMarkPrepareRequest.fromJSON(req.getContent().getInputStream());
			} else {
			...
			if (r.getDigitalSignatureArea()==null || r.getDigitalSignatureArea().equals("")) { 
				tracer.error("No DigitalSignatureArea specified for TimestampMarkPrepareWS action");
				throw new WebScriptException(Status.STATUS_BAD_REQUEST, "No DigitalSignatureArea specified for TimestampMarkPrepareWS action");
			}
			...
			Action documentTimestampMarkPrepare = actionService.createAction(DocumentTimestampMarkPrepare.ACTION_NAME_DOCUMENT_TIMESTAMP_MARK_PREPARE);
			documentTimestampMarkPrepare.setParameterValue(DocumentTimestampMarkPrepare.PARAM_MARK_AREA, r.getMarkDocumentArea());
			documentTimestampMarkPrepare.setParameterValue(DocumentTimestampMarkPrepare.PARAM_CLIENT_AREA, r.getDigitalSignatureArea());
			try {
			...
			TimestampMarkPrepareResponse resp = new TimestampMarkPrepareResponse();
			...
			tracer.error("Generic error on TimestampMarkPrepareWS web script : " + e.getClass().getName() + " : " + e.getMessage(),e);
			throw new WebScriptException(Status.STATUS_NOT_ACCEPTABLE, "Generic error on TimestampMarkPrepareWS web script : " + e.getClass().getName() + " : " + e.getMessage(),e);
		}
		
		if (tracer.isDebugEnabled()) tracer.debug("webscript TimestampMarkPrepareWS finished");
