...
import org.sinekarta.alfresco.webscript.request.AEMarkDocumentAddRequest;
import org.sinekarta.alfresco.webscript.response.AEMarkDocumentAddResponse;
...
public class AEMarkDocumentAddWS extends DeclarativeWebScript {

	private static final String REQUEST_TYPE = "requestType";

	private static Logger tracer = Logger.getLogger(AEMarkDocumentAddWS.class);
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		if (tracer.isDebugEnabled()) tracer.debug("webscript AEMarkDocumentAddWS starting");
		
		Map<String, Object> model = new HashMap<String, Object>();
		
		try {
			String requestType = req.getParameter(REQUEST_TYPE);
			AEMarkDocumentAddRequest r = null;
			if (requestType.equalsIgnoreCase("xml")) {
				r = AEMarkDocumentAddRequest.fromXML(req.getContent().getInputStream());
			} else if (requestType.equalsIgnoreCase("json")) {
				r = AEMarkDocumentAddRequest.fromJSON(req.getContent().getInputStream());
			} else {
			...
			if (r.getMarkDocumentArea()==null || r.getMarkDocumentArea().equals("")) { 
				tracer.error("No MarkDocumentArea specified for AEMarkDocumentAddWS action");
				throw new WebScriptException(Status.STATUS_BAD_REQUEST, "No MarkDocumentArea specified for AEMarkDocumentAddWS action");
			}
			...
			for (String nodeRefStr : r._getNodeRefs()) {
				...
				Action markDocumentAdd = actionService.createAction(AEMarkDocumentAdd.ACTION_NAME_AEMARK_DOCUMENT_ADD);
			...
			AEMarkDocumentAddResponse resp = new AEMarkDocumentAddResponse();
			...
			tracer.error("Generic error on AEMarkDocumentAddWS web script : " + e.getClass().getName() + " : " + e.getMessage(),e);
			throw new WebScriptException(Status.STATUS_NOT_ACCEPTABLE, "Generic error on AEMarkDocumentAddWS web script : " + e.getClass().getName() + " : " + e.getMessage(),e);
		}
		
		if (tracer.isDebugEnabled()) tracer.debug("webscript AEMarkDocumentAddWS finished");
